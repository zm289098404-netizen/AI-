...
### ✨ 功能 (Features)
- **内置 20 套行业模板**：覆盖政务/政企、金融、制造/工业、互联网/AI、行业应用、安全/运维、服务交付等 8 大类；模板管理与生成下拉均按行业分组折叠展示，支持关键词筛选与一键展开/收起。
- **登录页演示 / 正式部署双模式**：登录卡片提供 Tab 切换，演示模式显示一键填充快捷账号，正式部署模式展示安全清单与隐藏演示账号；管理员可在「系统设置」永久切换。
- **管理端全面重构**：抽出「概览 / AI 模型 / 租户与用户 / 系统设置 / 审计日志」五个子标签，加入系统概览卡片、AI 模型「测试连接」按钮、上线检查清单。
- **品牌可定制**：管理员可设置系统品牌名称（登录页、Header、浏览器标题同步）。
- **AI 连接测试 API**：`POST /api/admin/model-config/test` 用当前 Provider 发送一次最小请求，返回连通性结果与延迟。
- **公开系统信息接口**：`GET /api/system` 供登录前读取品牌名/部署模式（不含密钥）。

### 🎨 体验 (UX)
- 全新视觉：CSS 变量化主题、紫色辅色、卡片阴影与悬浮态、统一圆角、响应式（移动端可用）。
- 全局 Toast 通知组件，统一替代散落红字提示。
- 演示账号「点击即填」，登录页底部显示当前系统状态（部署模式 / Mock·Live / 检索后端）。

### 🧪 测试 (Tests)
- 测试增加至 80+ 项，覆盖 20 套模板 + 分类校验、部署模式 / 品牌设置、连接测试 API、子标签鉴权等。

### 📘 文档 (Docs)
- 新增 `docs/roadmap.md` / `docs/roadmap.en.md`：作为专业 AI 投标系统的企业级优化建议清单。
- README 中英双语同步更新功能列表与登录指南。

## [1.0.1-pre] - 管理端 AI 模型 / Mock 切换 / Provider
