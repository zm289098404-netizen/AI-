...
- 📐 **20 套行业模板（折叠分组）**：覆盖政务、金融、制造、互联网/AI、行业应用、安全/运维、服务交付等 8 大类，章节模板管理与生成下拉均按行业分组折叠，支持关键词筛选。
- 📤 **一键导出**：将生成的标书导出为 **Word(.docx)** 或 **PDF**（中文字体内嵌）。
- 🔐 **登录 + 多租户 + 演示/正式部署双模式**：账号体系 + 按租户隔离的知识库；登录页可切换「演示登录」与「正式部署登录」（演示模式提供快捷账号；正式模式展示安全清单并隐藏演示账号）。
- 🧾 **操作审计日志**（折叠面板 + 固定高度滚动）。
- 🤖 **AI 模型热切换 + 连接测试**：管理员可配置 Provider / Base URL / API Key（支持 Azure OpenAI、DeepSeek、通义千问、智谱、SiliconFlow、Kimi、自定义 OpenAI 兼容服务），切换 **Mock / 真实模型**（auto/强制开/强制关），并一键发起「测试连接」验证连通性。
- 🛠 **管理端专业化**：拆为「概览 / AI 模型 / 租户与用户 / 系统设置 / 审计日志」五个子标签 + 全局 Toast 通知 + 上线检查清单 + 可定制品牌名称。
- 🐳 **容器化**：提供 Dockerfile + docker-compose，一键本地起服务。
- 🔌 **Mock 模式**：未配置凭据时自动降级 **Mock 模式**，无需凭据即可演示全流程。
...
示例知识库中包含政务/金融/制造/智能客服等场景，并加入了基于公开资料归纳重写的**脱敏竞标模板**，便于直接体验 AI 生成标书的效果。
